#!/usr/bin/env bash
# DeepSeek-V4.1-Flash - Deploy + Verify (raat ko)
# KUBECONFIG=~/.kube/saif-config  ns=vllm
...
# 4) Wait worker -> 1/1 Running (load 552B, ~15-40 min)
...
# CUDA-graph/OOM: args mein add -> --max-model-len 262144 --max-num-seqs 128 --gpu-memory-utilization 0.9
# NCCL shm issue: sharedMemory 16Gi -> 20Gi
# unrecognized flag: logs grep check, wahi flag hatao dobara apply

# 8) Rollback (agar zaroorat ho)
